      ...
      {jobs.length === 0 ? (
        ...
        <>
          <p className="lead" style={{ marginTop: 0 }}>
            Druck-Vorgänge anklicken, um flux-Produkt und Overrides zu setzen.
          </p>
            ...
            <table className="data">
              ...
              <tbody>
                {jobs.map((j) => {
                  ...
                  const offen = expanded === j.id;
                  return (
                    <Fragment key={j.id}>
                      <tr
                        onClick={klickbar ? () => setExpanded(offen ? null : j.id) : undefined}
                        style={klickbar ? { cursor: "pointer", background: offen ? "var(--bg)" : undefined } : undefined}
                      >
                        <td><span className="tag">{j.typ}</span></td>
                        <td>
                          {klickbar && <span style={{ color: "var(--muted)", marginRight: 4 }}>{offen ? "▾" : "▸"}</span>}
                          {j.bauteil}
                      ...
                      {klickbar && offen && (
                        <tr>
                          <td colSpan={7} style={{ padding: "0 0 12px", background: "var(--bg)" }}>
                            <div style={{ padding: 12 }}>
                              <FluxFelder orderId={orderId} job={j} products={products} fluxUrlTpl={fluxUrlTpl} />
                            </div>
                          </td>
                        </tr>
                      )}
